...
def Turf_reg(request):
    if request.method=="POST":
        ...
        if turf_reg_tbl.objects.filter(temail=temail).exists():
          ...
          obj=turf_reg_tbl.objects.create(tname=tname,timg=timg,tdetails=tdetails,temail=temail,tmob=tmob,dist=dist,taddress=taddress,ps1=ps1,ps2=ps2)
          obj.save()
          if obj:
...
def turf_view_booking(request):
   tid=request.session['tid']
   obj=turf_booking_tbl.objects.filter(turf__id=tid)
   if obj:
